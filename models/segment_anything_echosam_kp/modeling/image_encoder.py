...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        # self.refine_Adapter = SingleConv(in_channels=embed_dim, out_channels=embed_dim)
        ...
        self.att_index = global_attn_indexes
        ...
        # self.layer_ffs = nn.ModuleList()
        # # for i in range(6):


        self.layer_ff_1 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ff_2 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ff_3 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ff_4 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ff_5 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ff_6 = LayerFeatureFusion(embed_dim,num_heads=num_heads)
        self.layer_ffs = [self.layer_ff_1,self.layer_ff_2,self.layer_ff_3,
                          self.layer_ff_4,self.layer_ff_5,self.layer_ff_6,]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for blk in self.blocks:
            if count < 7:
                cnnx = None
                x = blk(x, cnnx)
                vit_embedding = [x] + vit_embedding
                count += 1
            else:
                cnnx = None
                x = blk(x, cnnx)
                layer_ff = self.layer_ffs[aaa]
                x = layer_ff(x, vit_embedding[aaa])
                aaa += 1
...
class Layer_CrossAttn(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        ...
        x = (attn @ v).view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
        x = self.proj(x)
        # x = self.refine_Adapter(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

        return x

class LayerFeatureFusion(nn.Module):
    def __init__(self, dim, num_heads = 12):
        super(LayerFeatureFusion, self).__init__()
        self.Recombination = FeatureRecombination(dim, skip_connect=False)
        self.scale_matrix = torch.nn.Parameter(torch.randn(768), requires_grad=True)

        self.scale = 0

    def forward(self, deep, shallow):
        # F = deep + shallow * self.scale
        F = deep * self.scale_matrix + shallow
        out = self.Recombination(F)

        return out


class FeatureRecombination(nn.Module):
    def __init__(self, D_features, ratio=0.25, act_layer=nn.GELU, skip_connect=True):  # 0.25
        super().__init__()
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        # if self.depth > 6:
        #     self.mlp_ft = MLPBlock(embedding_dim=dim, mlp_dim=int(dim * mlp_ratio), act=act_layer)
        #     self.norm2_ft = norm_layer(dim)
        # else:
        ...
        # self.ReMLP = Adapter(dim, skip_connect=False)
        self.scale = 0.5
        self.dim = dim
        self.depth = depth

    def forward(self, x: torch.Tensor,cnnx) -> Tuple[torch.Tensor, torch.Tensor]:
        shortcut = x
        x = self.norm1(x)
        # Window partition
        if self.window_size > 0:
